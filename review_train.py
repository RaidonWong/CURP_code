...
def extract_input_description(input_str):
    ...
    格式: "Generate the review text written by a reviewer who has a given an overall rating of \"5.0\" for a product with description \"xxxxxxx.\". The summary of the review text is \"xxxxx\"."
    """
...
class ReviewWritingDataset(Dataset):
    """Review Writing数据集：从JSONL读取，实时编码profile"""
    def __init__(self, jsonl_path, encoder_model, encoder_tokenizer, llm_tokenizer, 
    ...
    def __getitem__(self, idx):
        ...
        input_text = item.get("input", "")  # 需要生成review的输入
        output_text = item.get("output", "")  # ground truth review text
        profile = item.get("profile", [])  # 用户历史profile
        
        # 构建profile文本并编码（review_writing格式：使用reviewText字段）
        user_embs_list = []
        for i, profile_item in enumerate(profile[:self.his_len]):
            review_text = profile_item.get("reviewText", "")
            
            # review_writing: 使用reviewText
            if review_text.strip():
                # 编码
                emb = self.encode_text(review_text)
                user_embs_list.append(emb)
            else:
                # 如果reviewText为空，使用零向量
        ...
        return {
            "input": input_text,  # 需要生成review的输入
            "output": output_text,  # ground truth review text
            "user_embeddings": user_embs  # (his_len, 1024)
        }


class ReviewWritingTrainer:
    """Review Writing训练器：训练MLP，LLM冻结"""
    ...
    def compute_loss(self, batch):
        """计算loss"""
        inputs = batch["input"]  # 需要生成review的输入列表
        outputs = batch.get("output", [""] * len(inputs))  # ground truth review text列表
        ...
        for i in range(batch_size):
            ...
            # 从input中提取查询信息（review_writing格式）
            query_text = get_query_review_writing(input_text)
            
            # 构建prompt（使用soft prompt方式）
            if his_len > 0:
...
def train(args):
    ...
    if accelerator.is_local_main_process:
        print("=" * 80)
        print("Review Writing Training: MLP Projection with PQ Codebook")
        print("=" * 80)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch in progress_bar:
            ...
            if args.save_steps > 0 and global_step % args.save_steps == 0:
                if accelerator.is_local_main_process:
                    ...
                    checkpoint_path = os.path.join(args.output_dir, f"review_writing_checkpoint-{global_step}.pt")
        ...
        if accelerator.is_local_main_process:
            ...
            checkpoint_path = os.path.join(args.output_dir, f"review_writing_checkpoint-epoch{epoch + 1}.pt")
            os.makedirs(args.output_dir, exist_ok=True)
            torch.save(checkpoint_dict, checkpoint_path)
...
def main():
    parser = argparse.ArgumentParser(description="Review Writing Training: MLP with PQ Codebook")
    
    # 数据参数
    parser.add_argument("--data_path", type=str,
                       default="/mnt/workspace/wangliang/review_writing/train_merged.jsonl",
